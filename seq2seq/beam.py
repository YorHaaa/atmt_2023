...
class BeamSearch(object):
    ...
    def get_best(self, regular_lambda):
        ...
        for _ in range(self.final.qsize()):
            node = self.final.get()
            final_score = node[2].logp - regular_lambda*node[2].squared_regular
            merged.put((-final_score, next(self._counter), node[2]))
...
class BeamSearchNode(object):
    ...
    def __init__(self, search, emb, lstm_out, final_hidden, final_cell, mask, sequence, logProb, length,
        ...
        self.logp = logProb
        self.length = length
        self.squared_regular = squared_regular
